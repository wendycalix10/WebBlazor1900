    ...
    public class UsuarioRepositorio : IUsuarioRepositorio
    {
        ...
        public async Task<bool> ActualizarAsync(Usuario user)
        {
            ...
            try
            {
                ...
                string sql = @"UPDATE usuario SET Nombre = @Nombre, Contrasena = @Contrasena, Correo = @Correo,
                               Rol = @Rol, Foto = @Foto, EstaActivo = @EstaActivo WHERE CodigoUsuario = @CodigoUsuario;";
        ...
        public async Task<bool> NuevoAsync(Usuario user)
        {
            ...
            try
            {
                using MySqlConnection _conexion = Conexion();
                await _conexion.OpenAsync();
                string sql = @"INSERT INTO usuario (CodigoUsuario, Nombre, Contrasena, Correo, FechaCreacion, Rol, Foto, EstaActivo) 
                                VALUES (@CodigoUsuario, @Nombre, @Contrasena, @Correo, @FechaCreacion, @Rol, @Foto, @EstaActivo);";
                resultado = Convert.ToBoolean(await _conexion.ExecuteAsync(sql, user));
            }
            catch (Exception)
            {
